...
Training data of the Router is built by running the trained GVAE from the last step on the next 120 data from the training dataset. This is controlled by `dataset_start_index`(which is the start of the data to train) and `num_of_data`(which is the number of the data used to train from start index). It still calls the same train_gsm8k.py, just the argument is different. A sample experiment script can be seen at [build_router_data_gsm8k.sh](https://github.com/szjiozi/GRouter/blob/main/scripts/build_router_data_gsm8k.sh) which will run run_gsm8k.py to generate the training data for router. Also remember to set the `dataset_json` to the path you want to use as the training data.
